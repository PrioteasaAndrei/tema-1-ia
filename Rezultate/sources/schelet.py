...
class HanoiTowers:
	...
	'''
		...
		Cele 4 turnuri sunt notate P, Q, R, G de la stanga la dreapta, G fiind goal
		Starea va fi un tuple de forma (a0,a1,a2...) unde ai este stiva pe care se afla discul de marime i (P,Q,R sau G)
		Starea de inceput va fi (P,P,P....)
		Starea de final va fi (G,G,G....)
	'''
	...
	'''
	Numarul de discuri care sunt la pe stiva finala
	'''
	@staticmethod
	def heuristic1(game):
	...
	'''
	Numarul de discuri care nu sunt pe stiva finala
	'''
	@staticmethod
	def heuristic2(game):
		a = game.r.count('G')
	...
	def __lt__(self, other):
		return True
	def __str__(self) -> str:
		return str(self.no_disks) + "-no disks:" + str(self.r) + "\n" + str([i for i in range(self.no_disks)])
	def __repr__(self) -> str: return str(self)	
...
class GLDS:
	...
	def iteration_iter(self,state,h,discovered,limit,start_time):
		curr = state
		to_be_popped = []
		mem_used = 0
		no_iterations = 0
		while True:
			succ = []
			for neigh in curr.get_neighbours():
				if neigh.r == neigh.solved().r:
					discovered[neigh] = (neigh.evaluate_state(h),curr)
					## pop la lista 
					for el in to_be_popped:
						discovered.pop(el)

					return (1 + no_iterations, len(succ) + 1 + mem_used)

				if neigh not in discovered:
					## (score,node,parent)
					heappush(succ,(neigh.evaluate_state(h),neigh,curr))
			...
			discovered[best_node] = (best_node.evaluate_state(h),curr)
			to_be_popped += [best_node]
			curr = best_node
			mem_used += len(succ)
			no_iterations += 1
	...
	def iteration(self,state,discrepancy,h,discovered,limit,start_time):
		...
		if discrepancy == 0:
			...
			tbr = self.iteration_iter(best_node,h,discovered,limit,start_time)
			...
			while len(succ) > 0:
				...
				discovered[next_best_node] = (next_score,next_best_parent)
	...
	def iteration(self,level,discrepancy,B,h,visited,limit,start_time):
		...
		if discrepancy == 0:
			...
			while already_explored < len(succ):
				...
				nivel_urm = nsmallest(already_explored + n,succ)[n:]
